...
async function parseWeather(loc) {
  ...
  load.style = "margin-top: 75px; font-size: 40px; background-color: #D4D4D4; border: 1px solid black; padding: 30px; position: absolute; bottom: 0; font-family: monospace; border-radius: 22px; margin-bottom: 30px;";
  load.style.color = "red"; //not used as is
  ...
  if (resp.desc1 === "Clouds") {
    //maybe could rewrite all this as function calls if you decide to add more on
  ...
  } else if (resp.desc1 === "Clear") {
    //&& new Date().toLocaleTimeString > a certain time, show night?
  ...
  text.innerHTML =
    ...
    resp.windDeg +
    " degrees (meteorological)<br>" +
    "cloudiness: " +
    resp.clouds +
    "%<br> time of data capture: " +
    resp.time +
    "<br> sunset: " +
    resp.sunset +
    "<br> sunrise: " +
...
async function changeLoc() {
  //everytime a user submits a different location, new API calls are run
  let load = document.querySelector(".loading");
  ...
  load.style.padding = "0px"; //gets rid of padding so it doesn't show after load
}
